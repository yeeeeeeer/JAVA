...
/*
    ...
    실질적인 데이터를 물리적으로 저장 x, 쿼리만 저장했지만
    테이블을 사용하는 것과 동일하게 사용할 수 있다.
    데이터를 쉽게 읽고 이해할 수 있도록 돕는 동시에 원본데이터의 보안을 유지하는데 도움이 된다.
    
    복잡한 쿼리문을 묶어서 조회하거나 재사용할 수 있다 / 변경은 못함
*/
...
INSERT INTO tbl_menu VALUES (NULL, '식헤맛국밥', 5500, 4, 'Y');

SELECT * FROM hansik;

-- view를 통한 DML(insert, update, delete) : 원본에 영향을 줌
...
-- 새로 만들거나, 기존에 있으면 이걸로 바꿔라 replace
