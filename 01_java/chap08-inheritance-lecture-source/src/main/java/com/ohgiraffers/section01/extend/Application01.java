...
public class Application01 {
    public static void main(String[] args) {
         ...
         * 클래스 또한 부모 클ㄹ래스와 자식 클래스로 역할을 나누어서 부모가 가지는 멤버를 자식이 물려 받아
         * 자기의 멤버인 것처럼 사용할 수 있도록 만든 기술이다.
         *
         * 하지만 단순 물려받는 개념보다는 조금 더 나아가서
         * 자바에서의 상속은 부모클래스의 확장(extend)의 개념을 가진다.
         * 물려받아서 자신의 것처럼 사용하는 것뿐만 아니라 추가적인 멤버도 작성이 가능하다.
         ...
         * 유지보수에 악영향을 미친다.
         * 2. 부모 클래스의 변경이 쉽지 않다. 자식 클래스에서 중요하게 사용하는 기능인 경우
         * 부모 클래스를 변경할 시 자식 클래스 모두 영향을 줄 수 있다.
         * 유지보수에 악영향을 미친다.
         * 3. 부모 클래스에서는 의미 있었던 기능이 자식 클래스에서는 무의미할 수 있다. (불필요한 기능)
         *
         * 장점, 단점을 고려했을 때
         * 상속은 재사용이라는 장점만 바라보게 되면 오용가능성이 있기때문에 유지보수에 좋지 않은 코드를 작성할 수 있다.
         ...
         * 적절한 책임을 가진 객체들이 서로 협력(메세지 수신과 송신)을 통해 프로그램이 동작되는 것이 객체지향 프로그래밍이다.
         *
         * 적절한 책임을 수행하는 객체 또한 그 객체만 수행할 수 있는 기능이라기보다, 역할적 관점으로 바라봐야 한다.
         * 역할이란 동일한 동작을 수행하는 것을 정의하며 대체 가능성을 의미한다
         * 부모클래스를 추상화 하는 경우에는 역할적 관점을 바라봐야 한다.
         * 그래야 자식 클래스로 생성한 객체들이 서로 다른 역할을 수행해가며 유연한 코드를 작성할 수 있게 된다.
        ...
        // 부모에게 자식을 넣어줄 수 있다 -> 자식에서 재정의한 메소드가 우선순위라 부모꺼보다 먼저 나온다.
        Car test = fireCar;   // 메소드에 쓸수 있다없다??
        System.out.println("자식을 부모에게 넣어줌");
        test.run();
        test.soundHorn();
