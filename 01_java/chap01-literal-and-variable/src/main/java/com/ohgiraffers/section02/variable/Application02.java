...
public class Application02 {
    public static void main(String[] args) {
        ...
        // int bonus;
        // System.out.println(bonus);     //변수 선언만하면 사용하지 못한다. //변수 중복 안됨.

        /*
         * 변수에는 자료형이 존재한다.
         * 자료형이란? 다양한 값의 형태별로 어느 정도의 크기를 하나의 값을 취급할 것인지 미리 컴파일러와 약속한 키워드이다.
         * 예) 앞서 사용한 int 자료형은 4byte(1byte=8bit) 만큼을 읽어서 하나의 값으로 취급하겠다는 약속이다.
        ...
        /*
         * 1 byte는 8개의 bit가 모여 만들어진 단위다.
         * 1 bit는 0과 1 둘 중 하나의 값만 가질 수 있는 데이터의 최소 단위이다.
         * 자바는 기본적으로 데이터의 최소 단위를 1 byte만큼 취급한다.
         * */
        ...
        short snum; // 2byte
        int inum; // 4byte     **
        long lnum; // 8byte     // 숫자뒤에 "l"을 적어줘야함.

        /* 실수를 취급하는 자료형 */
        float fnum; // 4byte     //숫자뒤에 "f" 적어줘야함.
        double dnum; // 8byte    **
        ...
        /*논리값을 취급하는 자료형*/
        /*1byte 논리 데이터형은 0과 1의 값으로 데이터를 취급하기때문에 1bit로 표현가능하지만, 최소단위가 1byte이기 때문에 1byte크기를 갖는다.*/
        boolean isTrue; //2byte
        ...
        /*
         * 목차 2. 변수에 값을 대입하기
         * 위에서 한 변수 선언은 메모리에 값을 저장하기 위한 공간만 생성해둔 상태이다.
         * 그 공간에 대입 연산자를 이용하여 자료형에 저장하기로 한 값을 저장할 수 있다.
         * 만약, 약속내용과 다른 형태의 값을 대입하려고 하면 컴파일러는 에러를 발생시킨다.
         *
         * 컴파일러: 자바실행 전 컴파일러가 에러 유무를 확인하기위해 읽는다.
         *
         * 설명
